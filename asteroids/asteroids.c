...
void update_asteroids(struct asteroid asteroids[], uint8_t size)
{
    ...
    for (uint8_t i = 0; i < size; ++i) {
        ...
        for (uint8_t j = 0; j < VERTS; ++j) {
            ...
            asteroids[i].world_vert[j].x = 0; asteroids[i].world_vert[j].y = 0;
            add_vector(&asteroids[i].world_vert[j], &asteroids[i].obj_vert[j]);
            add_vector(&asteroids[i].world_vert[j], &asteroids[i].location);
            add_vector(&asteroids[i].world_vert[j], &translation);
